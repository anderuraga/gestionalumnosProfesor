...
public class EmpleadoAspectAfter {
	@After("args(nombre)")
	public void losStringArgumentos(String nombre) {
		System.out
				.println("Se ejecuta despues del 'Advice'. Se pasa como argumento String="
						+ nombre);
	}

	@AfterThrowing("within(com.ipartek.formacion.aspect.anotaciones.Empleado)")
	public void logExcepciones(Joinpoint joinpoint) {
		System.out.println("Excepciones lanzadas en Empleado Metodo="
				+ joinpoint.toString());

	}

	@AfterReturning(pointcut = "execution(* getNombre())", returning = "retunString")
