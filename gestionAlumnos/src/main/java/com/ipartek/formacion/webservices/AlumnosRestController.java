...
@RestController	//propia de Spring 4. Equivale a @Controller y @ResponseBody
...
public class AlumnosRestController {
	...
	public ResponseEntity<Void> update(@PathVariable("id") int id, Alumno alumno){
		ResponseEntity<Void> respuesta=null;
		Alumno alum=as.getById(id);
		if(alum!=null||alumno.getCodigo()>-1){
			alumno.setCodigo(id);
			as.update(alumno);
