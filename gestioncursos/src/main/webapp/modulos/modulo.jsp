		...
		<form name="formulario" id="formulario" method="POST"
			...
			<div class="col-xs-6">
				<input type="hidden" id="${properties.parCodigo}"
					name="${properties.parCodigo}" value="<%=modulo.getCodigo()%>" />
				...
				<div class="form-group">
					<label for="${properties.parNombre}">NOMBRE: </label> <input
						...
						id="${properties.parNombre}" value="<%=modulo.getNombre()%>" />
				</div>
				<div class="form-group">
					<label for="${properties.parUFormativa}">UNIDAD FORMATIVA:
					</label> <input type="text" class="form-control"
						name="${properties.parUFormativa}"
						id="${properties.parUFormativa}"
						value="<%=modulo.getuFormativa()%>" />
				</div>
				<div class="form-group">
					<label for="${properties.parDuracion}">DURACIÓN: </label> <select
						class="form-control" name="${properties.parDuracion}"
						id="${properties.parDuracion}">
						<%
						for(DuracionHoras duracion: DuracionHoras.values()){
							%>
						<option
							<%=duracion.equals(modulo.getDuracion())? "selected" : "" %>
							value="<%=duracion.getCodigo()%>"><%=duracion.getDuracion()%></option>
						<%
						}
						%>
