		...
		<div class="row">
			<div class="col-xs-12">
				...
				<form action='<%=Constantes.SERVLET_MODULOS%>' method='POST' name='' id=''>
					<a class="col-xs-7"
						href='<%=Constantes.SERVLET_MODULOS%>?<%=Constantes.PAR_CODIGO%>=<%=modulo.getCodModulo()%>'><%=modulo.getUFormativa()%>
						- <%=modulo.getNombreModulo()%></a> <input type='hidden' id='<%=Constantes.PAR_CODIGO%>'
						name='<%=Constantes.PAR_CODIGO%>' value='<%=modulo.getCodModulo()%>' /> <input type='hidden'
