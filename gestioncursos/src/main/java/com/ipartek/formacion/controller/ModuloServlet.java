...
public class ModuloServlet extends HttpServlet {
	...
	private void recogerDatos(HttpServletRequest request) {
		modulo = new Modulo();
		recogerId(request);
		modulo.setCodModulo(id);
		String nombre = request.getParameter(Constantes.PAR_NOMBRE);
		modulo.setNombre(nombre);
		String referencia=request.getParameter(Constantes.PAR_REFERENCIA);
		modulo.setUFormativa(referencia);
		Duracion duraModulo=Util.parseDuracion(Integer.parseInt(request.getParameter(Constantes.PAR_DURACION)));
