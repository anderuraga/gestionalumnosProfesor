...
public class AlumnoServiceImp implements AlumnoService {
   ...
   *
   * alumno.setNombre("Imanol"); alumno.setApellidos("Jimenez Lopez"); alumnos.add(alumno); i++;
   *
  ...
  /*
   * private int getIndex(int codigo) throws AlumnoServiceException { int index = -1; int i = 0, len
   * = this.alumnos.size(); boolean encontrado = false; while (i < len && encontrado == false) {
   * Alumno aux = this.alumnos.get(i);// alumnos[i]; if (aux.getCodigo() == codigo) { encontrado =
   * true; index = i; } i++; } if (i == -1) { throw new AlumnoServiceException(
