...
public class Util {
	...
	public static Genero parseGenero(int gen){
	//	int cod = Integer.parseInt(gen);
		Genero genero=Genero.FEMENINO;;
		if (Genero.MASCULINO.getCodigo()==gen) {
			genero=Genero.MASCULINO;
		} else{
			if ( genero.OTROS.getCodigo() == gen){
				genero = Genero.OTROS;
			}
		}
		return genero;
	...
	public static TipoCurso parseTipoCurso(int tC){
		TipoCurso tipC=null;
		if (TipoCurso.LANBIDE.getCodigo()==tC) {
			tipC=TipoCurso.LANBIDE;
		}else if (TipoCurso.HOBETUZ.getCodigo()==tC) {
			tipC=TipoCurso.HOBETUZ;
		}else if (TipoCurso.FTRIPARTITA.getCodigo()==tC) {
			tipC=TipoCurso.FTRIPARTITA;
		}
		
		return tipC;
	}
	
	public static Duracion parseDuracion(int duracion){
		Duracion d=Duracion.QUINCE;
		//int val=Integer.parseInt(duracion);
		if (Duracion.VEINTE.getValor()==duracion) {
			d=Duracion.VEINTE;
		}else if (Duracion.CUARENTAYCINCO.getValor()==duracion) {
			d=Duracion.CUARENTAYCINCO;	
		}else if (Duracion.OCHENTA.getValor()==duracion) {
			d=Duracion.OCHENTA;
		}else if (Duracion.NOVENTA.getValor()==duracion) {
			d=Duracion.NOVENTA;
		}
