...
public class Util {
	...
	public static List<Genero> parseGenero(String[] generos){
		...
		for(int i=0; i<generos.length; i++){
			...
			int codigoGenero = Integer.parseInt(generos[i]);
	...
	public static DuracionModulo parseDuracion(String duracion){
		...
		if(codigo==DuracionModulo.HORAS20.getCodigo()){
			...
			if(codigo==DuracionModulo.HORAS30.getCodigo()){
				d = DuracionModulo.HORAS30;
			} else{
	...
	public static TipoCurso parseTipoCurso(String codigo){
		...
		if(TipoCurso.FORMACION_TRIPARTITA.getCodigo()==cod){
			tipo = TipoCurso.FORMACION_TRIPARTITA;
		}else{
	...
	public static TipoCurso parseTipoCurso(int codTipo) {
	    TipoCurso tipo = TipoCurso.LANBIDE;
	    if (codTipo == TipoCurso.HOBETUZ.getCodigo()) {
	      tipo = TipoCurso.HOBETUZ;
	    } else {
	      if (codTipo == TipoCurso.FORMACION_TRIPARTITA.getCodigo()) {
	        tipo = TipoCurso.FORMACION_TRIPARTITA;
	      }
	    }
